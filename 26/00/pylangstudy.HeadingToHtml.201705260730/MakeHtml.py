...
class Main(object):
    ...
    def __MakeHtml(self, tsv):
        ...
        with open('ContentsView.html', mode='w', encoding='utf-8') as f:
            f.write(html_str)
    
    def __CreateHtmlHeader(self):
        return '<link rel="stylesheet" href="ContentsView.css">'
    """
    def __CreateAggregateTable(self):
        return ('<tr><th>進捗率</th><td>{0} % ({1}/{2})</td></tr>'.format(self.__aggregate.ProgressRate, (self.__aggregate.Finished + self.__aggregate.ZeroFinished), self.__aggregate.Total) + 
            '<tr><th title="完了">完</th><td>{0}</td></tr>'.format(self.__aggregate.Finished) + 
#            '<tr><th>完了(成果物なし)</th><td>{0}</td></tr>'.format(self.__aggregate.ZeroFinished) + 
#            '<tr><th>成果物なし</th><td>{0}</td></tr>'.format(self.__aggregate.ZeroFinished) + 
            '<tr><th title="完了 成果物なし">果無</th><td>{0}</td></tr>'.format(self.__aggregate.ZeroFinished) + 
            '<tr><th>未完</th><td>{0}</td></tr>'.format(self.__aggregate.Unfinished))
    """
    def __CreateAggregateTable(self):
        return ('<tr><th>進捗率</th><td>{0} % ({1}/{2})</td></tr>'.format(self.__aggregate.ProgressRate, (self.__aggregate.Finished + self.__aggregate.ZeroFinished), self.__aggregate.Total) + 
...
#            return '<img src="http://www.google.com/s2/favicons?domain={0}"><a href="{2}" title="{1}"></a></img>'.format(urllib.parse.urlparse(row[2]).netloc, row[2], os.path.basename(os.path.dirname(row[2])))
